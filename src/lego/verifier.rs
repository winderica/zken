...
pub fn verify_qap_proof<E: Pairing>(
    ...
    let hash =
        Blake2b::new().chain(to_bytes!(&a)).chain(to_bytes!(&b)).chain(to_bytes!(&delta_prime));
    let m_fr = E::ScalarField::from_le_bytes_mod_order(&hash.finalize());

    let delta_prime_delta_m = pvk.vk.delta_g2.mul(m_fr) + delta_prime;

    let qap = E::multi_miller_loop(
        [a, c, d],
        [b.into(), delta_prime_delta_m.neg().into_affine().into(), pvk.gamma_g2_neg_pc.clone()],
    );
...
pub fn verify_proof<E: Pairing>(
    ...
    verify_qap_proof(pvk, proof.a, proof.b, proof.c, d.into_affine(), proof.delta_prime)
}
