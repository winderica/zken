...
pub fn generate_random_parameters_incl_cp_link<E, C, R>(
    ...
    rng: &mut R,
) -> Result<ProvingKeyWithLink<E>, SynthesisError>
where
    E: Pairing,
    ...
    QAP: R1CStoQAP,
{
    ...
    for i in 0..commit_witness_count {
        link_m.insert_row_slice(i, i, vec![link_gens.g]);
        link_m.insert_row_slice(i, i + commit_witness_count, vec![link_gens.h]);
    }
    ...
    let vk = VerifyingKeyWithLink::<E> {
        ...
        link_pp,
        link_bases: (link_gens.g, link_gens.h),
        link_vk,
    };
...
fn generate_parameters_and_extra_info_with_qap<E, C, R, QAP>(
    ...
    commit_witness_count: usize,
    rng: &mut R,
) -> Result<(ProvingKey<E>, usize), SynthesisError>
where
    E: Pairing,
    C: ConstraintSynthesizer<E::ScalarField>,
    R: Rng,
    QAP: R1CStoQAP,
{
    ...
    let non_zero_a: usize =
        cfg_into_iter!(0..qap_num_variables).map(|i| usize::from(!a[i].is_zero())).sum();

    let non_zero_b: usize =
        cfg_into_iter!(0..qap_num_variables).map(|i| usize::from(!b[i].is_zero())).sum();
    ...
    let vk = VerifyingKey::<E> {
        ...
        commit_witness_count,
        zt_delta_g1: h_query[0].into_affine(),
    };
...
fn generate_randomness<E, R>(
    rng: &mut R,
) -> (E::ScalarField, E::ScalarField, E::ScalarField, E::ScalarField, E::ScalarField, E::G1, E::G2)
where
    E: Pairing,
    R: Rng,
{
