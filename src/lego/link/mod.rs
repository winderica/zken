...
mod test {
    ...
    fn test_basic() {
        ...
        let mut pp = PP::<Bn254> { l: 1, t: 2, g1, g2 };
        ...
        let (ek, vk) = PESubspaceSnark::<Bn254>::keygen(&mut rng, &pp, &m);

        let pi = PESubspaceSnark::<Bn254>::prove(&mut pp, &ek, &x);
        let pi_bad = PESubspaceSnark::<Bn254>::prove(&mut pp, &ek, &x_bad);

        assert!(PESubspaceSnark::<Bn254>::verify(&pp, &vk, &y, &pi));
        assert!(!PESubspaceSnark::<Bn254>::verify(&pp, &vk, &y, &pi_bad));
    }
    ...
    fn test_basic_1() {
        ...
        let pi = PESubspaceSnark::<Bn254>::prove(&mut pp, &ek, &w);
        let pi_bad = PESubspaceSnark::<Bn254>::prove(&mut pp, &ek, &w_bad);
    ...
    fn test_same_value_different_bases() {
        ...
        let mut pp = PP::<Bn254> { l: 2, t: 3, g1, g2 };
        ...
        assert!(PESubspaceSnark::<Bn254>::verify(&pp, &vk, &x, &pi));
    }
    ...
    fn test_some_vals_equal() {
        ...
        let w: Vec<Fr> =
            vec![Fr::rand(&mut rng), Fr::rand(&mut rng), Fr::rand(&mut rng), Fr::rand(&mut rng)];
