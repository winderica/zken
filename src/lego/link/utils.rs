...
impl<T: Copy> SparseMatrix<T> {
    ...
    pub fn new(nr: usize, nc: usize) -> SparseMatrix<T> {
        SparseMatrix { cols: vec![vec![]; nc], nr, nc }
    }
...
pub fn inner_product<PE: Pairing>(a: &[<PE::ScalarField as PrimeField>::BigInt], b: &[PE::G1Affine]) -> PE::G1Affine {
    PE::G1::msm_bigint(b, &a).into_affine()
}
